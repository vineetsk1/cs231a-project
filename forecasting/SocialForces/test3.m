...
%                     d2 = d2 + (d2==0);
                    ...
                    Ti(1:npedi,1:2) = dest(d1,:)+5*randn(npedi,2) ;% initial position + noise
                   % Ti(1:npedi,1:2) = dest(d1,:)+[5*randn(npedi,1) zeros(npedi,1) ];
                    ...
                    TrS = [TrS; Ti(:,[4 1 2]) ...    % label + px + py
                        Vinit...               % initial speed
                        dest(d2,:)+5*randn(npedi,2)  ...       % final destination
            ...
            if ~isempty(TrS)
                ...
                %
                                ...
                                plot((Vid(:,2)),(Vid(:,3)),'o');
                                hold on
                                plot(round(10.*Vid((Vid(:,7)==20),2)),round(10.*Vid((Vid(:,7)==20),3)),'ro');% current agents' position
                                plot(round(10.*Vid((Vid(:,7)==40),2)),round(10.*Vid((Vid(:,7)==40),3)),'go');
                                %plot(round(10.*Vid((Vid(:,7)==0),2)),round(10.*Vid((Vid(:,7)==0),3)),'ro');
                                % plot(Obst(:,1),Obst(:,2),'k.')      % map of Obstacles
                                plot(cen(1),cen(2),'r*')            % barycenter of destinations
                                plot(dest(:,1),dest(:,2),'b*')
                                axis([-5 50 -5 50]);% final destinations & entrances
...
%                 %             frame = getframe;
        ...
        % cd ..
        cd /scr/alexr/simtrajRdSF_Eccv4/
