function [ err,errd, Pred, Grp ] = simError( Obs, Sims, method, varargin )
...
for i = 1:2:length(varargin)
    ...
    if strcmp(varargin{i},'BehaviorClassifier'), SVMclass = varargin{i+1}; end
    if strcmp(varargin{i},'Clust'),clust = varargin{i+1}; end
...
if strcmp(method,'ewap') || strcmp(method,'attr') || strcmp(method,'attrmc')
    if isempty(params) || isempty(Dest), error('Missing arguments'); end
end
if strcmp(method,'attrmc')
    if isempty(SVMclass) || isempty(clust), error('Missing arguments'); end
end
...
for i = 1:size(Sims,1)
    ...
    for j = 1:length(time)-1
        ...
        elseif strcmp(method,'attrmc')
            ...
            groups = logical(arrayfun(@(x) g{j}(g{j}(:,3)==x,5),...
                D{i}(ind,3)));
            I = Ecollision(theta(1),theta(2:end),vhat(j,:),...
                [phat(j,:);o;D{i}(ind,4:5)],[vhat(j,:);zeros(size(o));D{i}(ind,6:7)]);
            xx = [pid time(j) phat(j,:) vhat(j,:) pdest(dhat(j+1),:) u(j+1)];
            c=svmpredict(1,xx,SVMclass);
            if I>0.15
                theta(1)=double(clust(c));
            else
                theta(1)=params(1);
            end
            ...
            vhat(j+1,:) = ...
                ...
                pdest(dhat(j+1),:),...              % zi
                theta,...                           % params
                [true;false(size(o,1),1);groups]),... % group indicator                          % class
                vhat(j,:),...                           % init value
                optimset('GradObj','on',...
                'LargeScale','off',...
                'Display','off'...
    ...
    errd{i} = sqrt(sum((p(end,:) - phat(end,:)).^2,2));
    %total error
