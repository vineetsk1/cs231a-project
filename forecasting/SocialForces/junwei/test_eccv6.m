...
%         Is = data.mask./9;
        Im = imresize(data.background,0.2);
        dest = data.dest02;
        path = data.path;
        LocalGoal = data.lg;
        c = data.c;
...
%             end
            if size(crop,1)<65
                crop = [crop ;  zeros(65-size(crop,1),size(crop,2),3)];
            end
            if size(crop,2)<65
                crop = [crop  zeros(65,65-size(crop,2),3)];
            end
           crop = imresize(crop,0.5);
            Crop=[Crop;crop(:)'];
        end
        ...
        % cd ..
        cd /scr/alexr/simtrajRdSF_Eccv5_3/
