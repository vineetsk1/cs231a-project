        ...
        while N>0 || t<lt
            ...
            if t<=lt
                ...
                for k=1:length(d1)
                    ...
                    d = sum(abs(Ti(k,1:2)-cen));
%                     if sum(abs(Ti(k,1:2)-destloc{k+Nind})) < d
%                         destloc{k+Nind} = [destloc{k+Nind};cen];
%                     else
%                         destloc{k+Nind} = [cen; destloc{k+Nind}];
%                     end
                    if c(k)>0
                        destloc{k+Nind}=[];
                    else
                        destloc{k+Nind}= 0.7*destloc{k+Nind}+0.3*cen;
                    end
                    
                end

                Pedstart(Pedstart(:,3)==t,4)=Pedstart(Pedstart(:,3)==t,4)./((c*2)+1);
...
%                                          pause(0.001)
        ...
        % cd ..
        cd /scr/alexr/simtrajRdSF_Eccv3/
